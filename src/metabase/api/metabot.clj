...
(defn- infer-sql-or-throw
  ...
  [context question table_id]
  (or
    (metabot/infer-db-native-sql-query context table_id)
    (throw
      (let [message (format
...
(api/defendpoint-schema POST "/database/:database-id"
                        ...
                        [database-id :as {{:keys [question table_id]} :body}]
                        ...
                        (let [{:as database} (api/check-404 (t2/select-one Database :id database-id))
                          ...
                          ;(if-some [context]
                            (let [dataset (infer-sql-or-throw context question table_id)]
                              dataset)
                            ;(throw
                            ;  (let [message (format
                            ;                  (str/join
